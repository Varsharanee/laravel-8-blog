...
<div>
    <div class="" id="">
        <h1> <img class="post_img" src="{{asset('uploads/thumbnail_image/'.$detail_post->thumbnail)}}"
                alt="{{$detail_post->title}}"><br></h1>
    </div>
</div>
...
<article>
    <div class="m-5 justify-content-center">
        ...
        <div class="post-preview">
            ...
            <img class="post-img" src="{{asset('uploads/post_image/'.$detail_post->full_image)}}"
                alt="{{$detail_post->title}}">
        ...
        <div class="my-5">
            ...
            <form id="contactForm" method="POST"
                action="{{url('save-comment/'.Str::slug($detail_post->title).'/'.$detail_post->id)}}">
                @csrf
                <div class="form-floating">
                    <textarea name="comment" class="form-control" id="message" placeholder="Enter your message here..."
        ...
        <div class="card my-4">
            <h5 class="card-header">Comments <span
                    class="badge rounded-pill bg-dark">{{count($detail_post->comments)}}</span></h5>
            <div class="card-body">
                @if(count($detail_post->comments)>0)
                @foreach($detail_post->comments as $comment)
                <blockquote class="blockquote">
                    
                    <button class="btn btn-sm">Delete Comment</button>
